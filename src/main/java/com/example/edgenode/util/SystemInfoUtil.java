...
public class SystemInfoUtil {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Logger LOG = LoggerFactory.getLogger(SystemInfoUtil.class);
...
//		System.out.println("  release date: " + (firmware.getReleaseDate() == null ? "unknown": firmware.getReleaseDate() == null ? "unknown" : FormatUtil.formatDate(firmware.getReleaseDate())));
    ...
    private static void printNetworkInterfaces(HardwareAbstractionLayer hal1) throws InterruptedException {
        System.out.println("网络信息:");
        long startTime = 0L;
        long endTime = 0L;
        long recvBytesStart = 0L;
        long recvBytesEnd = 0L;
        for(int i = 0;i<2;i++){
            SystemInfo si = new SystemInfo();
            HardwareAbstractionLayer hal = si.getHardware();
            ...
            for (NetworkIF net : networkIFS) {
                ...
                if(i == 0){
                    startTime = net.getTimeStamp();
                    recvBytesStart = net.getBytesRecv();
                    System.out.format(" 获取的字节数: %s", recvBytesStart);
                    Thread.sleep(2000);

                }
                if(i == 1){
                    endTime = net.getTimeStamp();
                    recvBytesEnd = net.getBytesRecv();
                    System.out.format(" 获取的字节数: %s", recvBytesEnd);
                }
        ...
        System.out.println();
        System.out.format(" 字节数差: %s", recvBytesEnd - recvBytesStart);
        System.out.format(" 时间差: %s", endTime-startTime);
        System.out.println(" 网速:"+ ((recvBytesEnd-recvBytesStart)*1.0)/((endTime-startTime)*1.0)+"KB/s");
